...
const SAMPLE = {
  ...
  selectedFlightPrice: 1150,
  selectedSeatPrice: 50,
};

export function CreateReservation({ reservation = SAMPLE }) {
  const hasSeatPrice = reservation?.selectedSeatPrice !== undefined;
  return (
        ...
        <div className="flex flex-col justify-between gap-4">
          ...
          {hasSeatPrice && (
            <div className="flex flex-col gap-1 text-xs text-muted-foreground">
              <div>Flight price: ${reservation?.selectedFlightPrice ?? "-"}</div>
              <div>Seat price: ${reservation?.selectedSeatPrice ?? "-"}</div>
              <div className="font-semibold text-foreground">Total: ${reservation.totalPriceInUSD}</div>
            </div>
          )}
